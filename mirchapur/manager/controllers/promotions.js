...
.config(['$qProvider', function ($qProvider) {
  ...
  .controller('promotionsController', function($scope, $http, $interval, $cookies) {
      ...
      $scope.createCoupon = function(){
        ...
        data.percentageP = $scope.newCoupon.percentageP;    
      	...
      	if(!$scope.editCouponWindowFlag && $scope.newCouponWindowFlag){
      		myurl = "https://accelerateengine.app/foodengine/services/createcoupon.php";      	
      	}
      	else if($scope.editCouponWindowFlag && !$scope.newCouponWindowFlag){
      		data.permanentCode = $scope.viewingCode;
      		myurl = "https://accelerateengine.app/foodengine/services/editcoupon.php";
      	}
	...
	var data = {};
        ...
        $http({
          method  : 'POST',
          url     : 'https://accelerateengine.app/foodengine/services/deactivatecoupon.php',
          data    : data,
          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
         })
         .then(function(response) {
  	...
  	if(type == 'RANGE'){
  		if(document.getElementById("reportFromDate").value == '' || document.getElementById("reportToDate").value == ''){
  			...
  			window.open ("https://accelerateengine.app/foodengine/services/fetchcouponsummary.php?access="+temp_token+"&uid="+uid+"&mode="+mode+"&from="+from+"&to="+to);
  			$('#dateModal').modal('hide');
  		}
  	}
  	else{
  		window.open ("https://accelerateengine.app/foodengine/services/fetchcouponsummary.php?access="+temp_token+"&uid="+uid+"&mode="+mode+"&from=&to=");
  		$('#dateModal').modal('hide');
  	}
