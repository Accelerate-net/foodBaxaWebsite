...
$scope.initVouchers = function(){
      ...
      $http({
        method  : 'POST',
        url     : 'https://accelerateengine.app/foodengine/services/fetchvouchersadmin.php',
        data    : data,
...
$scope.initVouchers();
	...
	document.getElementById("reportToDate").value = todaySetDate;
  	...
  	if(type == 'RANGE'){
  		if(document.getElementById("reportFromDate").value == '' || document.getElementById("reportToDate").value == ''){
  			...
  			window.open ("https://accelerateengine.app/foodengine/services/fetchvouchersummary.php?access="+temp_token+"&from="+from+"&to="+to);
  			$('#dateModal').modal('hide');
  		}
  	}
  	else{
  		window.open ("https://accelerateengine.app/foodengine/services/fetchvouchersummary.php?access="+temp_token+"&from=&to=");
  		$('#dateModal').modal('hide');
  	}
         ...
         $scope.confirmCancel = function(code){
	        ...
	        $http({
	          method  : 'POST',
	          url     : 'https://accelerateengine.app/foodengine/services/cancelvoucheradmin.php',
	          data    : data,
	          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
	         })
	         .then(function(response) {
	...
	$scope.checkAvailability = function(){	
		...
		if($scope.newVoucherContent.code != ''){
		        ...
		        $http({
		          method  : 'POST',
		          url     : 'https://accelerateengine.app/foodengine/services/customvouchercheck.php',
		          data    : data,
		          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
		         })
		         .then(function(response) {
	...
	$scope.confirmVoucher = function(){
			var data = {};
		        ...
		        $http({
		          method  : 'POST',
		          url     : 'https://accelerateengine.app/foodengine/services/newvoucheradmin.php',
		          data    : data,
		          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
		         })
		         .then(function(response) {
