...
.config(['$qProvider', function ($qProvider) {
	...
	if(document.getElementById('myNewLocationWindow').style.display == "block"){
      ...
      $scope.updateOutlet = function(myLocationCode){
	      ...
	      $http.get("https://accelerateengine.app/foodengine/services/fetchoutletsweb.php?locationCode="+myLocationCode).then(function(response) {
	...
	$scope.closeLocation();
     ...
     $scope.initTypeahead = function(){
	     ...
	     var areaSuggestions = new Bloodhound({
	        ...
	        remote: {
	          url: 'https://accelerateengine.app/foodengine/services/searchareas.php?city='+mycity+'&key=%QUERY',
	          wildcard: '%QUERY'
	        }
	      });
    ...
    $http.get("https://accelerateengine.app/foodengine/services/fetchmenuweb.php?outlet="+outletcode).then(function(response) {
        $scope.menu = response.data;
    });
