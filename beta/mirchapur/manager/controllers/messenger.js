...
.config(['$qProvider', function ($qProvider) {
  ...
  .controller('messengerController', function($scope, $http, $interval, $cookies) {
	...
	$.get("https://accelerateengine.app/foodengine/services/fetchclassinfosimple.php", function(data){
	...
	$scope.deleteContent = function(req_id){
		var co_data = {};
	        ...
	        $http({
	          method  : 'POST',
	          url     : 'https://accelerateengine.app/foodengine/services/deletemarketingcontent.php',
	          data    : co_data,
	          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
	         })
	         .then(function(response) {
		...
		if(type == 'push'){ //1. Push
			...
			if($scope.addNewContent.title == ""){
		      	...
		      	if($scope.addNewContent.brief == ""){
			        ...
			        $http({
			          method  : 'POST',
			          url     : 'https://accelerateengine.app/foodengine/services/pushcampaigncreate.php',
			          data    : data,
			          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
			         })
			         .then(function(response) {
			...
			if($scope.addNewContent.brief == ""){
			        ...
			        $http({
			          method  : 'POST',
			          url     : 'https://accelerateengine.app/foodengine/services/smscampaign.php',
			          data    : data,
			          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
			         })
			         .then(function(response) {
	...
	$scope.submitSMS = function(){				
			...
			if($scope.addNewContent.brief == ""){
			        ...
			        $http({
			          method  : 'POST',
			          url     : 'https://accelerateengine.app/foodengine/services/smscampaigncreate.php',
			          data    : data,
			          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
			         })
			         .then(function(response) {
