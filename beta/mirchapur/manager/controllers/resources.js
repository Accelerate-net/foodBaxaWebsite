...
.config(['$qProvider', function ($qProvider) {
    ...
    $scope.logoutNow = function(){
      ...
      $scope.addAgent = function(){
        ...
        if(data.code == "" || data.name == ""){
          ...
          $http({
            method  : 'POST',
            url     : 'https://accelerateengine.app/foodengine/services/addagent.php',
            data    : data,
      ...
      $scope.removeAgent = function(code){
        ...
        $http({
          method  : 'POST',
          url     : 'https://accelerateengine.app/foodengine/services/removeagent.php',
          data    : data,
          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
         })
         .then(function(response) {
  ...
  .controller('financeledgerController', function($scope, $http, $interval, $cookies) {
	...
	$scope.callSales = function(){
		...
		if(document.getElementById("quickFromDate").value == ''){
			...
			$scope.isWaitingResponse = true;
		        ...
		        $http({
		          method  : 'POST',
		          url     : 'https://accelerateengine.app/foodengine/services/dailysales.php',
		          data    : data,
		          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
		         })
		         .then(function(response) {
	...
	$scope.callSales();
	    ...
	    if(mode == 'SALES'){
            	window.open ("https://accelerateengine.app/foodengine/services/fetchledger.php?access="+temp_token+"&from="+temp_from+"&to="+temp_to);
            }
            else if(mode == 'DISCOUNTS'){
            	window.open ("https://accelerateengine.app/foodengine/services/fetchledgerdiscounts.php?access="+temp_token+"&from="+temp_from+"&to="+temp_to);
            }
