...
.controller('CheckoutController', ['$scope','$http', '$cookies', function($scope, $http, $cookies) {
   ...
   if(localStorage.getItem("pickupOutlet")){
    ...
    $scope.onOutletChange = function(outlet){
      ...
      $http.get('https://accelerateengine.app/foodengine/services/fetchoutlets.php?outletcode='+outlet)
    ...
    if(!$scope.isCouponApplied){
      ...
      $scope.checkout = function(checkoutAddress, comments, coupon){
	    ...
	    $('#loading').show();
            ...
            $http({
              method  : 'POST',
              url     : 'https://accelerateengine.app/foodengine/services/createorder.php',
              data    : data,
              headers : {'Content-Type': 'application/x-www-form-urlencoded'}
             })
              .then(function(response) {
				    	...
				    	if(timeLeft == 0)
				    	{
				    		...
				    		setTimeout(function(){
				    			...
				    			if(attemptsCount == 5){
				    				...
				    				document.getElementById("paymentNote").innerHTML = "Contact hello@foodbaxa.com with Reference ID ("+data.transactionID+") for refunding the debited amount.";
	                            ...
	                            $http({
	                              method  : 'POST',
	                              url     : 'https://accelerateengine.app/foodengine/services/processpayment.php',
	                              data    : data,
	                              headers : {'Content-Type': 'application/x-www-form-urlencoded'},
	                              timeout : 18000
	                             })
