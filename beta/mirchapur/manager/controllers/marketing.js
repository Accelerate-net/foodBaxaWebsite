    ...
    $scope.logoutNow = function(){
      ...
      $scope.initializeContent = function(type, req_pageid, option){
		...
		//Fetch Active Content
	        ...
	        $http({
	          method  : 'POST',
	          url     : 'https://accelerateengine.app/foodengine/services/fetchmarketingcontent.php',
	          data    : co_data,
		...
		if(type == 'coupons'){ //1. Coupon 
			...
			if($scope.addNewContent.code == ""){
				...
				var myExpiryDate = dd+'-'+mm+'-'+yyyy;
			        ...
			        $http({
			          method  : 'POST',
			          url     : 'https://accelerateengine.app/foodengine/services/newmarketingcontent.php',
			          data    : data,
			          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
			         })
			         .then(function(response) {
